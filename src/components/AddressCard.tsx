...
export const AddressCard = ({ icon: Icon, label, style }: AddressCardProps) => {
  const dataRef = useRef<HTMLDataElement>(null)

  return (
    <span class='inline-card' role='figure' style={style}>
      <span role='img'><Icon /></span>
      <data ref={dataRef} class='truncate text-legible' value={label}>{label}</data>
      <span>
        <span role='img'><Icon /></span>
        <span role='menu'>
          <data class='truncate text-legible' value={label}>{label}</data>
          <span>
            <button type='button' value='copy' role='menuitem' tabIndex={1}>
            ...
            <button type='button' value='edit' role='menuitem' tabIndex={1}>
